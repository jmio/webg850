...
struct Timing {
	...
	/* --- 送出中に USB の割り込みを止めるか ---
	 *
	 * 1 なら短い H の間だけ USB の割り込みを止める（既定）。
	 *
	 * 送出はビジーウェイトなので、USB の ISR が H の最中に走ると H が
	 * その分だけ伸びる。ビット 0 の 162usec に対して判定のしきい値は
	 * 284usec しかなく、122usec 伸びればビット 1 に化ける。優先度では
	 * 直せない（こちらは割り込みで動いていない）ので止める。
	 *
	 * 0 にすると止めない。効いているかを比べるためのもの。
	 * 詳細は usbirq.h を参照。
	 */
	uint8_t  usb_mask;

	/* 止める H の上限 [usec]。これを超える H では止めない。
	 *
	 * 区切りの 30msec まで止めると USB を長く待たせることになり、
	 * そこでは幅の精度も要らない。ビット 1 の 406usec は含めたいので
	 * 既定は 1000。
	 */
	uint32_t usb_mask_max_us;

	/* --- 送出中に受信を捌くか ---
	 *
	 * 1 なら L の区間ごとに受信バッファを読み捨てる（既定）。
	 *
	 * 読まないと相手のバッファが詰まり、**送出中はホストから何も
	 * 送れなくなる**。usbmask の効きを試すのに雑音を流したとき、
	 * 17.6 秒のうち最初の 0.4 秒しか届いていなかった（狙い 352828
	 * バイトに対して 8000 バイト）。捌くまで負荷試験が成立しない。
	 *
	 * 送出しながらホストから .bin を食べる（12288 バイト制限を外す）
	 * ための下ごしらえでもある。読むのは L の間だけにして、遅れが
	 * H の幅に乗らないようにする。
	 */
	uint8_t  play_pump;
};
