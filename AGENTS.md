...
解析の成果は `docs/` 以下に Markdown で残します。

| 場所 | 内容 |
|:--|:--|
| [docs/](docs/) 直下 | 解析結果をまとめたドキュメント |
| [docs/plans/](docs/plans/) | 解析計画（何を・どの手順で・どこまで調べるか）|
| `docs/binary/` | **追跡対象外。** ROM (Z80 バイナリ) の解析経過のドキュメントと作業ファイル（逆アセンブル結果・ダンプ）|
...
（`.gitignore` で除外済み）。コミットするのは ROM の中身に踏み込まない範囲のドキュメントだけです。

解析経過は次の流れで `docs/` 直下へ移します。

1. 解析中は `docs/binary/` に書く（追跡対象外。ROM の内容に踏み込んでよい）
2. 一区切りついたら、ROM の内容に直接依存しない部分を抜き出してリライトする
3. **リライトした内容の確認を得てから** `docs/` 直下へ移す

`docs/binary/` の内容をそのままコピーしないこと。確認を経ずに `docs/` 直下へ移動・コミットしないこと。
